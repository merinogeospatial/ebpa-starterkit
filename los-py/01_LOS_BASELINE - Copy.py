...
def route_data_mile(route, park, block):
    ...
    arcpy.AddField_management(route_tbl, "ACRE", "DOUBLE", "", "", "", "ACRE")
    arcpy.AddField_management(route_tbl, "PARK", "DOUBLE", "", "", "", "PARK")
    ...
    arcpy.AddField_management(site_tbl, "ACRE", "DOUBLE", "", "", "", "ACRE")
    arcpy.AddField_management(site_tbl, "PARK", "DOUBLE", "", "", "", "PARK")
    expression6 = "(!MEAN_ACRES!)"
    expression7 = "(1)"
    arcpy.CalculateField_management(site_tbl, "ACRE", expression6, "PYTHON_9.3")
    arcpy.CalculateField_management(site_tbl, "PARK", expression7, "PYTHON_9.3")

    arcpy.AddJoin_management(route_tbl, "SITE", site_tbl, "SITE")
    expression8 = "(!" + site_tbl + ".ACRE!)"
    expression9 = "(!" + site_tbl + ".PARK!)"
    arcpy.CalculateField_management(route_tbl, "ACRE", expression8, "PYTHON_9.3")
    arcpy.CalculateField_management(route_tbl, "PARK", expression9, "PYTHON_9.3")
    arcpy.RemoveJoin_management(route_tbl)

    # Summarize route layer by GEOID
    geoid_tbl = str(route_tbl) + "_geoidStats"
    arcpy.Statistics_analysis(route_tbl, geoid_tbl, [["ACRE", "SUM"], ["PARK", "SUM"]], "GEOID10")

    # join back to block and calculate fields
    arcpy.AddJoin_management(block, "GEOID10", geoid_tbl, "GEOID10")
    expression10 = "(!" + geoid_tbl + ".SUM_ACRE!)"
    expression11 = "(!" + geoid_tbl + ".SUM_PARK!)"
    arcpy.CalculateField_management(block, "ACRE", expression10, "PYTHON_9.3")
    arcpy.CalculateField_management(block, "PARK", expression11, "PYTHON_9.3")
    arcpy.RemoveJoin_management(block)

    with arcpy.da.UpdateCursor(block, ["ACRE", "PARK"]) as cursor:
        for row in cursor:
            if row[0] is None:
                row[0] = 0
            if row[1] is None:
...
del cursor
# Pull Values
field_names = ["GEOID10", "DIST", "ACRE", "PARK"]  # Field names list for search cursor
